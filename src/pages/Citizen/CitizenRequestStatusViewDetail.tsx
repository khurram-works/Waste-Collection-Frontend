...
const STATUS_ORDER = ["PENDING", "ASSIGNED", "COLLECTED", "VERIFIED", "PAID"] as const;
...
function StatCell({ label, value, highlight = false }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="bg-white rounded-lg border border-zinc-100 p-3">
      ...
      <p className={`text-sm font-bold mt-1 ${highlight ? "text-primary" : "text-zinc-800"}`}>{value}</p>
    </div>
  );
}

export default function RequestDetailsModal({ isOpen, onClose, request, totalearnings }) {
  const [cancelled, setcancelled] = useState(false);
  if (!request) return null;

  const {
    requestId, wasteType, requestDate, scheduledDate, status,
    estimatedWeight, estimatedEarnings, pickupAddress, citizenNote,
    photoUrl, actualWeight, rateApplied, condition,
    workerId, workerName, workerPhone, routeSchedule,
  } = request;

  const isPostCollection = status === "COLLECTED" || status === "VERIFIED" || status === "PAID";
              ...
              <div className="px-7 pt-7 pb-6 border-b border-zinc-100 flex items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="text-xl font-bold text-zinc-900">
                    Request <span className="font-mono text-primary">{requestId}</span>
                  </h2>
                  <p className="text-xs text-zinc-400 mt-1">
                    Submitted {new Date(requestDate).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}
                  </p>
                </div>
                <button
                  onClick={onClose}
                  className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-zinc-100 text-zinc-400 hover:text-zinc-600 transition-colors flex-shrink-0"
                >
              ...
              <div className="overflow-y-auto max-h-[calc(90vh-220px)] px-7 py-6 space-y-7">
                    ...
                    {STATUS_ORDER.map((step) => {
                      ...
                      return (
                        <div key={step} className="flex flex-col items-center gap-2 flex-1">
                          ...
                          <span className={`text-[10px] font-semibold text-center uppercase tracking-wider leading-tight
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 pt-1">
                  <div className="space-y-4">
                    <p className="text-[11px] uppercase tracking-widest font-bold text-zinc-300">Request Info</p>
                    <InfoRow icon="recycling"      label="Waste Type"       value={wasteType} />
                    <InfoRow icon="weight"         label="Estimated Weight" value={estimatedWeight} />
                    <InfoRow icon="calendar_today" label="Scheduled Date"   value={
                      scheduledDate
                        ? new Date(scheduledDate).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })
                        : "Not scheduled yet"
                    } />
                    <InfoRow icon="location_on" label="Pickup Address" value={pickupAddress} />
                  </div>

                  <div className="space-y-4">
                    <p className="text-[11px] uppercase tracking-widest font-bold text-zinc-300">Notes & Media</p>
                    <div className="bg-zinc-50 rounded-xl border border-zinc-100 p-4">
                      <p className="text-[10px] uppercase tracking-wider font-semibold text-zinc-400 mb-2">Citizen Note</p>
                      <p className="text-sm text-zinc-600 italic leading-relaxed">
                        {citizenNote ? `"${citizenNote}"` : <span className="not-italic text-zinc-300">No notes provided</span>}
                      </p>
                    </div>

                    {photoUrl && (
                      <div>
                        <p className="text-[10px] uppercase tracking-wider font-semibold text-zinc-400 mb-2">Photo</p>
                        <div className="relative group w-20 h-20 rounded-xl overflow-hidden border-2 border-zinc-100 hover:border-primary transition-all cursor-pointer shadow-sm">
                          ...
                          <img src={photoUrl} alt="Waste" className="w-full h-full object-cover" />
                        </div>
                      </div>
                    )}
                  </div>
               ...
               {isPostCollection && (
                <div className="bg-zinc-50 rounded-2xl border border-zinc-100 p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-bold text-zinc-800">Verification Details</h3>
                    </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                    <StatCell label="Actual Weight"    value={isPostCollection ? `${actualWeight} kg` : "—"} />
                    <StatCell label="Condition"        value={isPostCollection && condition ? condition.toLowerCase() : "—"} highlight />
                    <StatCell label="Rate Applied"     value={isPostCollection ? `Rs ${rateApplied}/kg` : "—"} />
                    <StatCell label="Est. Earnings"    value={`Rs ${estimatedEarnings}`} />
                    <StatCell label="Final Earnings"   value={status === "VERIFIED" ? `Rs ${estimatedEarnings}` : "—"} highlight />
                  </div>

                  {!isPostCollection && (
                    <p className="mt-4 text-xs text-center text-zinc-350 italic">
                      Verification details will appear once waste is collected.
                    </p>
                  )}
                </div>
                )}
                ...
                {workerId !== null && (
                  <div>
                    <p className="text-[11px] uppercase tracking-widest font-bold text-zinc-300 mb-3">Assigned Worker</p>
                    <div className="flex items-center justify-between bg-zinc-50 border border-zinc-100 rounded-xl p-4 gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-zinc-800">
                            {workerName ?? "—"}
                            <span className="text-zinc-400 font-normal text-xs ml-1">({workerId ?? "—"})</span>
                          </p>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="material-symbols-outlined text-zinc-400" style={{ fontSize: 13 }}>call</span>
                            <p className="text-xs text-zinc-500">{workerPhone ?? "—"}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      {isPostCollection && (
                        <div className="text-right flex-shrink-0">
                          <p className="text-[10px] uppercase tracking-wider font-semibold text-zinc-400">Est. Arrival</p>
                          <p className="text-sm font-bold text-zinc-800 mt-0.5">
                            {routeSchedule ? routeSchedule.slice(0, 16) : "—"}
                          </p>
                        </div>
                      )}
                    </div>
